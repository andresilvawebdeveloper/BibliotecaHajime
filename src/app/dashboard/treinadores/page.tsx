...
export default function TreinadoresPage() {
  ...
  const aprovarTreinador = async (id: string) => {
    ...
    else fetchTreinadores(); // Atualiza a lista
  };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F8FAFC', fontFamily: 'sans-serif' }}>
      <header style={headerStyle}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <h1 style={{ fontSize: '18px', fontWeight: '900' }}>Gestão de Treinadores</h1>
        </div>
        <Users size={24} color="#0055A4" />
      </header>
            ...
            <div key={t.id} style={userCardStyle(t.aprovado)}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {!t.aprovado && <span style={pendingBadgeStyle}>PENDENTE</span>}
                </div>
                <div style={userInfoStyle}>
                  <Mail size={12} /> {t.email}
                  <Calendar size={12} style={{ marginLeft: '10px' }} /> {t.data_nascimento || '---'}
                </div>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                {t.id !== perfilAdmin.id && (
                  <button onClick={() => eliminarPerfil(t.id, t.nome)} style={deleteBtnStyle}>
                    <Trash2 size={18} />
                  </button>
                )}
              </div>
...
const headerStyle: React.CSSProperties = { backgroundColor: 'white', padding: '15px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #E5E7EB' };
...
const userCardStyle = (aprovado: boolean): React.CSSProperties => ({
  backgroundColor: 'white', padding: '20px', borderRadius: '20px', border: aprovado ? '1px solid #E5E7EB' : '2px solid #0055A4',
  display: 'flex', alignItems: 'center', gap: '15px', boxShadow: aprovado ? 'none' : '0 4px 12px rgba(0,85,164,0.1)'
});
const avatarStyle = (isAdmin: boolean): React.CSSProperties => ({ width: '48px', height: '48px', borderRadius: '14px', backgroundColor: isAdmin ? '#0055A4' : '#F1F5F9', color: isAdmin ? 'white' : '#94A3B8', display: 'flex', alignItems: 'center', justifyContent: 'center' });
const userInfoStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: '5px', color: '#64748B', fontSize: '12px', marginTop: '4px' };
const pendingBadgeStyle: React.CSSProperties = { backgroundColor: '#FEE2E2', color: '#B91C1C', fontSize: '9px', fontWeight: '900', padding: '2px 6px', borderRadius: '4px' };
const approveBtnStyle: React.CSSProperties = { backgroundColor: '#0055A4', color: 'white', border: 'none', padding: '8px 16px', borderRadius: '10px', fontWeight: '700', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' };
const deleteBtnStyle: React.CSSProperties = { background: 'none', border: 'none', color: '#94A3B8', cursor: 'pointer' };